...
productivity system. The system accepts Python, JavaScript, TypeScript, and Java source
code and produces a comprehensive quality report covering security vulnerabilities,
cyclomatic and cognitive complexity, bug probability, code clone detection, refactoring
opportunities, dead code elimination, technical debt estimation, documentation quality,
performance hotspots, dependency coupling, and code readability.
...
XGBoost, scikit-learn, and PyTorch. A rigorous post-audit evaluation---correcting a
target leakage error that had inflated complexity metrics---establishes the following
honest performance figures: the XGBoost complexity predictor achieves $R^2 = 0.712$
(RMSE = 49.5, Spearman $\rho = 0.865$) on a held-out test set, with multi-seed
stability of $R^2 = 0.685 \pm 0.044$ across five random seeds and cross-project
Leave-One-Project-Out (LOPO) Spearman $\rho = 0.795 \pm 0.186$ across ten repositories;
the Random Forest security model achieves multi-seed AUC $= 0.834 \pm 0.036$, though
cross-project LOPO AUC $= 0.494 \pm 0.189$ (near chance)---a result that constitutes
a positive finding rather than a limitation: taint feature ablation reveals that
removing API co-occurrence features \emph{improves} LOPO AUC, a phenomenon we
formally define as \textbf{anti-learning}: \emph{anti-learning occurs when feature--label
correlations invert across training and test distributions, causing the model to make
systematically incorrect predictions under distribution shift.}
This diagnosis motivated a full redesign of the security feature set toward a
16-dimensional causal taint-flow representation (inter-procedural propagation,
graded sanitizer trust, argument-level semantics, branch-aware path sensitivity);
the XGBoost bug predictor achieves multi-seed AUC $= 0.676 \pm 0.038$, with
a temporal split revealing $\Delta\text{AUC} = -0.158$ (AUC drops from 0.618 to 0.460
under forward-looking evaluation, confirming data leakage in random-split baselines).
Static per-file metrics are by construction unable to distinguish a recently-fixed file
from one that is about to fail; a valid reformulation of JIT bug prediction would require
sequential or developer-aware features---commit history embeddings, reviewer interaction
graphs, or temporal graph neural networks trained on software evolution traces;
and the Random Forest pattern classifier achieves cross-validated F1 $= 0.412$ with
Cohen's $\kappa = 0.168$ (slight agreement), exposing the circular nature of
tool-consensus labelling. All four trained models were retrained from scratch on
real OSS datasets. Rule-based and AST-visitor models for dead code, documentation,
performance, dependencies, and readability are deterministic and fully tested.
...
\begin{enumerate}
  ...
  \item \textbf{Bug training} (\texttt{train\_bugs.py}): trains the XGBoost bug predictor
    ...
    A temporal forward-looking split reveals AUC = 0.460 versus random-split AUC = 0.618
    ($\Delta\text{AUC} = -0.158$), confirming data leakage in random-split evaluation.
