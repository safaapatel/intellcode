"""
Rust metric extraction via tree-sitter.
"""
...
_RUST_BOOL_OPS = frozenset(["&&", "||"])

_RUST_STRUCTURAL = frozenset([
    "if_expression", "while_expression", "for_expression",
    "loop_expression", "match_expression", "if_let_expression",
    "while_let_expression",
])

_RUST_NESTING_RAISERS = frozenset([
    "if_expression", "while_expression", "for_expression",
    "loop_expression", "match_expression", "if_let_expression",
    "while_let_expression", "closure_expression",
    "function_item", "impl_item",
])

_RUST_FUNC_TYPES = frozenset([
...
def compute_rust_metrics(source: str):
    ...
    try:
        ...
        result.cyclomatic_complexity = tree_cyclomatic(root, _RUST_BRANCH_TYPES, _RUST_BOOL_OPS)
        result.cognitive_complexity = tree_cognitive(
            root, _RUST_STRUCTURAL, _RUST_NESTING_RAISERS, _RUST_FUNC_TYPES, _RUST_BOOL_OPS,
        )

        func_ccs = []
        for fn_node, start, end in _iter_functions(root):
            fc = tree_cyclomatic(fn_node, _RUST_BRANCH_TYPES, _RUST_BOOL_OPS)
            func_ccs.append(fc)
            if (end - start + 1) > 50:
        ...
        import re
        keywords = ("if ", "while ", "for ", "loop ", "match ")
